// Last updated: 22/12/2025, 10:59:38 am
1class Solution {
...
25    //     return true;
26    // }
27   if(s.length()<2) return "";
28   Set<Character> st=new HashSet<>();
29   for(int i=0;i<s.length();i++){
30    st.add(s.charAt(i));
31   }
32   for(int i=0;i<s.length();i++){
33    char c=s.charAt(i);
34    if(st.contains(Character.toLowerCase(c)) && st.contains(Character.toUpperCase(c))){
35        continue;
36    }
37    String right=longestNiceSubstring(s.substring(0,i));
38    String left=longestNiceSubstring(s.substring(i+1));
39    return right.length()>=left.length()?right:left;
40   }
41return s;
42}
43}